...
useHead({ title: 'María Grandury — Award Proposal B' })
...
onMounted(async () => {
  try {
    const res = await fetch('/data/award_proposal_b.md')
    BIO.value = parseMarkdown(await res.text())
  } catch (e) { console.error('Error loading award_proposal_b.md', e) }
})
...
.aw-tooltip {
  ...
  padding: 10px 14px; min-width: 260px; max-width: 400px;

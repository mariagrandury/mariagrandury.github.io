...
useHead({ title: 'María Grandury — Award Proposal C' })
...
onMounted(async () => {
  try {
    const res = await fetch('/data/award_proposal_c.md')
    BIO.value = parseMarkdown(await res.text())
  } catch (e) { console.error('Error loading award_proposal_c.md', e) }
})
...
.aw-num-lg { font-size: 27px; font-weight: 800; color: #F5A623; min-width: 50px; line-height: 1; letter-spacing: -0.03em; flex-shrink: 0; font-variant-numeric: tabular-nums; }
.aw-num-sm { font-size: 17px; font-weight: 700; color: #E59010; min-width: 34px; line-height: 1; flex-shrink: 0; }
...
.aw-tip-icon { font-size: 11px; color: #00918A; margin-left: 3px; vertical-align: super; line-height: 1; }
.aw-tooltip {
  display: none;
  position: absolute; bottom: calc(100% + 6px); left: 0;
  background: #fff; border: 1.5px solid #00C9B1; border-radius: 8px;
  padding: 10px 14px; min-width: 260px; max-width: 420px;
